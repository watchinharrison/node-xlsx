...
function xlsx(file) { 
   ...
   if (typeof file === 'string') { // Load
		...
		while (w--) { 
			...
			if (worksheet.hidden) worksheets.unshift('<sheet name="' + (escapeXML(worksheet.name) || 'Sheet' + id) + '" sheetId="' + id + '" state="veryHidden" r:id="rId' + id + '"/>');
			else worksheets.unshift('<sheet name="' + (escapeXML(worksheet.name) || 'Sheet' + id) + '" sheetId="' + id + '" r:id="rId' + id + '"/>');
		}
